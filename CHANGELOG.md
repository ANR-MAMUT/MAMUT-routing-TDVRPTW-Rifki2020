...
**All 71 optimality certificates re-derived from scratch and re-stamped** under the four-solve agreement protocol (cold and warm starts crossed with the two labeling modes, an audited exact-pricing phase in every run, zero checker-infeasible priced columns) on Grid'5000, on the repaired kayros 1.5.1 build. The campaign was motivated by the 2026-08-05 withdrawal of one certificate in the Vu2020 TDVRPTW family and the subsequent finding that the certifying builds carried since-repaired pricing defects (documented in the kayros 1.5.1 release notes and CHANGELOG). Every re-issued stamp certifies bit-exactly the previously stored value; no solution data changed, and the stamps' provenance now cites this campaign and build.

## 2026-07-18

**Family-wide re-validation on the exact value-jump solver path; no data changes.** Full-family sweep (all 180 instances, n=10 to n=60) on the kayros 1.1.0 release candidate (commit f8c788c), in which the exact value-jump (mollifier-free, tagged-vertical) labeling path is enabled automatically on stepwise travel-time functions and the near-vertical continuization of the retracted 2026-07-08 era is retired from the pipeline entirely. Protocol unchanged from the 2026-07-10/12 campaigns: four independent exact solves per instance (cold and warm starts crossed with the two labeling modes; time limits 600 s up to n=50, 3600 s at n=60), Grid'5000 reservation 6781269. Results: 720 runs, zero checker-infeasible priced columns, and zero disagreements with the stored data of any kind. 59 of the 71 certified BKS were independently re-certified at their stored proven optima (four-run agreement within 4e-12 of the stored cost); every remaining run ended as an honest time-limit verdict, and no run anywhere contradicted a stored cost or certificate. The 22 instances of this family refused for checker-infeasible pricing by the 2026-07-10 wave (n=30 to n=60) now run clean: zero such events in their 88 runs, uncertified only because the time limit intervened. Costs and certificates are untouched: this entry records that the family's certificates now stand re-validated on the production solver path.
